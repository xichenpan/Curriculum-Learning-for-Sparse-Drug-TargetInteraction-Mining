...
def Decomposefull_without_label(csvfile, relevantdir):
    data = pd.read_csv(csvfile).values.tolist()
    drug, target = list(zip(*data))
    ...
    for drug, target in data:
        drug_id = drug2idx[drug]
        target_id = target2idx[target]
        pairs.append([drug_id, target_id])
    ...
    with open(relevantdir + 'data/output_pos_pairs.pkl', 'wb') as f:
        pickle.dump(pairs, f)

    with open(relevantdir + 'data/output_neg_pairs.pkl', 'wb') as f:
        pickle.dump(neg_pairs, f)
